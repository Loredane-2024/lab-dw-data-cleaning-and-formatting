...
def initial_format (df: pd.DataFrame):
    ...
    #df.st.columns =["state"]
    df = df.rename(columns={"st": "state"}, inplace=True)
    return df
...
def change_dtype (df: pd.DataFrame):
    ...
    df["number_of_open_complaints"] = df["number_of_open_complaints"].apply (lambda x: x.split("/")[1]if isinstance(x, str) else None)
